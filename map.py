...
def init():
    ...
    goal_y = largeur - 20 # bottom right near-corner
    first_update = False

# Initializing the last distance from car to the goal
last_distance = 0
...
class Car(Widget):
    ...
    def move(self, rotation):
        ...
        self.sensor1 = Vector(30, 0).rotate(self.angle) + self.pos # sensor are actually close to the car and must be always updated to be at the same locations near the car
        self.sensor2 = Vector(30, 0).rotate((self.angle+30)%360) + self.pos # in Vector(30, 0) 30 is actually the distance to the car
        self.sensor3 = Vector(30, 0).rotate((self.angle-30)%360) + self.pos
        self.signal1 = int(np.sum(sand[int(self.sensor1_x)-10:int(self.sensor1_x)+10, int(self.sensor1_y)-10:int(self.sensor1_y)+10]))/400.
        self.signal2 = int(np.sum(sand[int(self.sensor2_x)-10:int(self.sensor2_x)+10, int(self.sensor2_y)-10:int(self.sensor2_y)+10]))/400.
        self.signal3 = int(np.sum(sand[int(self.sensor3_x)-10:int(self.sensor3_x)+10, int(self.sensor3_y)-10:int(self.sensor3_y)+10]))/400.
        if self.sensor1_x>longueur-10 or self.sensor1_x<10 or self.sensor1_y>largeur-10 or self.sensor1_y<10: #if the car is too close to the map's borders a denser signal is given 
            self.signal1 = 1.
...
class Game(Widget):
    ...
    def update(self, dt):
        ...
        distance = np.sqrt((self.car.x - goal_x)**2 + (self.car.y - goal_y)**2) # update the distance to the goal
        self.ball1.pos = self.car.sensor1  # update the
        self.ball2.pos = self.car.sensor2  # positions of the
        self.ball3.pos = self.car.sensor3  # sensors

        if sand[int(self.car.x),int(self.car.y)] > 0: # if the car is inside sand,
            self.car.velocity = Vector(1, 0).rotate(self.car.angle) # its velocity is set to 1, normally its velocity is 6
            last_reward = -3 # the reward taken is -1 which is the worst
        else: # otherwise
            self.car.velocity = Vector(6, 0).rotate(self.car.angle)
            last_reward = -0.1 # if car is getting away from the goal a small penalty is given
